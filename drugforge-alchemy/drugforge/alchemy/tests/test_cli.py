...
def test_alchemy_prep_run_bad_chemistry(tmpdir):
    ...
    with warnings.catch_warnings(record=True) as w:
        with tmpdir.as_cwd():
            ...
            # note, it appears that will occasionally fail, producing 3/14 successful poses, instead of 4
            # there must be something deterministic going on, likely the 3d coordinates generated by rdkit
            # but it will resolve itself by running again
            assert (
                "[✓] Pose generation successful for 4/14." in result.stdout
            ), "This may sporadically fail; try rerunning. "
...
def test_alchemy_prep_run_from_postera(
    ...
    from drugforge.alchemy.cli import utils
