...
def test_cross_docking_cli_structure_directory_du_cache(
    ...
    """Test the ``cross-docking`` CLI command with a structure directory and design-unit cache."""
    runner = CliRunner()
...
def test_non_asap_target(tyk2_complex, tyk2_ligands, du_cache, tmp_path):
    """Test the ``cross-docking`` CLI command with a non-ASAP target, using a tyk2 PDB file and ligand SDF as input."""
    runner = CliRunner()
    du_cache_dir, _ = du_cache
    result = runner.invoke(
        cli,
